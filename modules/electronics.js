...
// modules/electronics.js
import { chromium } from "playwright";
import fs from "fs";
import path from "path";
import fetch from "node-fetch";
import { PROXIES, DISCORD_WEBHOOK_URL, ELECTRONICS_URL } from "../config.js";


// <<< NEW: Define the script name for logging
const SCRIPT_NAME = 'electronics.js';
...
// Path to sales data
const SALES_FILE = path.join(process.cwd(), "electronics.json");
  ...
  while (true) {
    ...
    while (trackedItems.length === 0 && attempt <= 5) {
      ...
      try {
        ...
        const response = await page.goto(ELECTRONICS_URL, { waitUntil: "domcontentloaded", timeout: 30000 });
