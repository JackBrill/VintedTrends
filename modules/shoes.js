...
import { PROXIES, DISCORD_WEBHOOK_URL, SHOES_URL } from "../config.js";
...
// Path to sales data
const SALES_FILE = path.join(process.cwd(), "shoes.json");
  ...
  while (true) {
    ...
    while (trackedItems.length === 0 && attempt <= 5) {
      ...
      try {
        ...
        const response = await page.goto(SHOES_URL, { waitUntil: "domcontentloaded", timeout: 30000 });
