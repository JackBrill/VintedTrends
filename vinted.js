...
class VintedDashboard {
  constructor() {
    ...
    this.activeFiltersContainer = document.getElementById('activeFiltersContainer'); // Get the new container
  ...
  renderSales() {
    ...
    // NEW: Render the active filter tags
  ...
  /**
   * NEW: Renders the active filter tags above the sales grid.
   */
  renderActiveFilters() {
  ...
  setupEventListeners() {
    ...
    // NEW: Event listener for removing active filter tags
    this.activeFiltersContainer.addEventListener('click', e => {
        ...
        if (removeBtn) {
            ...
            // Remove the value from the active filters array
