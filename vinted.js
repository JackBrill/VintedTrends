// This is the upgraded version of your script with modern anti-bot features.
...
// Settings
const BATCH_SIZE = 200; // You can now track more items
const MAX_PAGES_TO_SCAN = 10; // It will scan up to 10 pages to find items
...
async function collectItems() {
    ...
    while (collectedItemData.length < BATCH_SIZE && attempt <= 5) {
        ...
        // Using headless: false to run in a visible browser on the server via Xvfb
    ...
    while (true) {
        ...
        const runChecksWithWorkerPool = async () => {
            ...
            for (let i = 0; i < CONCURRENT_CHECKS; i++) {
                ...
                const context = await browser.newContext({
                    proxy: { server: `http://${proxy.host}:${proxy.port}`, username: proxy.user, password: proxy.pass },
                    userAgent: "Mozilla/5.o (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36",
                    locale: 'en-GB',
                    timezoneId: 'Europe/London',
                });
                const page = await context.newPage();
